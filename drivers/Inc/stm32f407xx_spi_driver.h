...
typedef struct{
	...
	uint8_t			*pTxBuffer;		//Store the app- TX buffer addr
	uint8_t			*pRxBuffer;		//Store the app- RX buffer addr
	uint32_t		TxLen;			//Store the Tx Len
	uint32_t		RxLen;			//Store the Rx Len
	uint8_t			TxState;		//Store the Tx state @SPI_STATES
	uint8_t			RxState;		//Store the Rx state @SPI_STATES
}SPI_Handle_t;
...
/*
 * Possible API Application events
 */
